...
function Completed() {
    ...
    const resolve_Item = async () => {
        ...
        try {
            ...
            const completedApi = await fetch(`http://localhost:4000/show_item_completed?id=${JSON.parse(localStorage.getItem('id'))}`)

            const completedata = await completedApi.json()
            console.log(completedata)
            setlist(completedata.tasks)   
